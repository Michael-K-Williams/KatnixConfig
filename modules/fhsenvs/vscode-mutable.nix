...
let
  ...
  vscode-activation-script = ''
    ${pkgs.writeShellScript "install-vscode" ''
      ...
      cat > /home/${userName}/.local/share/applications/vscode-mutable.desktop << EOF
[Desktop Entry]
Name=Visual Studio Code (Mutable)
Comment=Code Editing. Redefined.
GenericName=Text Editor
Exec=vscode-mutable %F
...
Exec=vscode-mutable --new-window %F
Icon=/opt/vscode-mutable/resources/app/resources/linux/code.png
EOF
      
      # Set proper ownership for desktop entry
      chown ${userName}:users /home/${userName}/.local/share/applications/vscode-mutable.desktop
      ...
      echo "VS Code installed to $VSCODE_DIR and available as 'vscode-mutable'" >&2
    ''}
  '';
...
in
{
  ...
  system.activationScripts.vscode-mutable = vscode-activation-script;
}